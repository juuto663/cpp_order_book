...
public:
	...
	{
		return std::make_shared<Order>(type, GetOrder(), GetSide(), GetPrice(), GetQuantity())
	}
private:
	OrderId orderId_;
	Price price_;
...
public:
	...
	{
		if (orders_.contains(order->GetOrder()))
			return { };
		...
		{
			auto& orders = asks[order->GetPrice()];
			orders.push_back(order);
			iterator = std::next(orders.begin(), orders.size() - 1);
		}

		orders_.insert({ order -> getOrderId(), OrderEntry{ odrer, iterator } });
		return MatchOrders();
	}
