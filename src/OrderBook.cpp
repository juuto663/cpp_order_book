...
private:
  ...
  Trades MatchOrders()
  {
    ...
    while (true)
    {
      ...
      // Copy the key first: bidPrice/askPrice reference the node being erased.
      if (bids.empty())
      {
...
public:
  ...
  void CancelOrder(OrderId orderId)
  {
    ...
    // Copy, not a reference: the entry is erased from orders_ below.
  ...
  Trades ModifyOrder(OrderModify order)
  {
    ...
    // Read the order type out before cancelling, which destroys the entry.
    const OrderType orderType = orders_.at(order.GetOrderId()).order_->GetOrderType();
    CancelOrder(order.GetOrderId());
